 ...
 *
 * Fetches and manages the student dashboard summary from the Spring Boot
 * backend. Exposes loading, error and data states to the consuming component.
 *
 * Usage:
 *   const { data, loading, error, refresh } = useDashboard(studentId);
 */

import { useCallback, useEffect, useState } from "react";
import { getDashboardSummary, ApiError } from "../api/index";
...
export function useDashboard(studentId: number | null | undefined): UseDashboardState {
  ...
  useEffect(() => {
    ...
    async function fetchData() {
      ...
      try {
        const result = await getDashboardSummary(studentId!);
