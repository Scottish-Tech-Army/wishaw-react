...
export default function AdminUsersPage() {
  ...
  const [form, setForm] = useState({ username: '', password: '', displayName: '', role: '', centreId: '' });
  ...
  return (
    <div>
      ...
      <table className="data-table desktop-only">
        <thead>
          <tr><th>ID</th><th>Username</th><th>Display Name</th><th>Role</th><th>Centre</th><th>Actions</th></tr>
        </thead>
        <tbody>
          {filtered.map((u) => (
            <tr key={u.id}>
          ...
          {filtered.length === 0 && <tr><td colSpan={6} className="text-center text-light">No users found</td></tr>}
        </tbody>
      </table>
