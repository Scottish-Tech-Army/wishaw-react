...
export const progressApi = {
  ...
  getCompletedSubBadgeIds: (userId) => api.get(`/progress/${userId}/completed-sub-badges`),
...
// ── Legacy Points ────────────────────────────────
export const legacyPointsApi = {
  getAll: () => api.get('/legacy-points'),
  getById: (id) => api.get(`/legacy-points/${id}`),
  getByUser: (userId) => api.get(`/legacy-points/user/${userId}`),
  create: (data) => api.post('/legacy-points', data),
  update: (id, data) => api.put(`/legacy-points/${id}`, data),
  remove: (id) => api.delete(`/legacy-points/${id}`),
};
